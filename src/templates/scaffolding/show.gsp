			...
			<%  excludedProps = Event.allEvents.toList() << 'id' << 'version'
				...
				<g:if test="\${${propertyName}?.${p.name}}">
                    ...
                    <%  } else if (p.type == Date || p.type == java.sql.Date || p.type == java.sql.Time || p.type == Calendar) { %>
                        <span class="property-value" aria-labelledby="${p.name}-label"><g:formatDate date="\${${propertyName}?.${p.name}}" formatName="datetime.date.format"  /></span>
                    <%  } else if (p.type == org.joda.time.LocalDate || p.type == org.joda.time.LocalTime ) { %>
                        <span class="property-value" aria-labelledby="${p.name}-label"><joda:format value="\${${propertyName}?.${p.name}}" 
					<%      if (cp?.format) { %>
						pattern="${cp?.format}"
                    <%      } %>
						/></span>
